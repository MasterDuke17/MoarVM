...
MVMint64 MVM_string_index(MVMThreadContext *tc, MVMString *haystack, MVMString *needle, MVMint64 start) {
    ...
    if (haystack->body.storage_type == MVM_STRING_GRAPHEME_8 && needle->body.storage_type == MVM_STRING_GRAPHEME_8) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_8 + start, hgraphs - start, needle->body.storage.blob_8, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("88 r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
    }
	else if (haystack->body.storage_type == MVM_STRING_GRAPHEME_8 && needle->body.storage_type == MVM_STRING_GRAPHEME_ASCII) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_8 + start, hgraphs - start, needle->body.storage.blob_ascii, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("8a r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
    }
	else if (haystack->body.storage_type == MVM_STRING_GRAPHEME_8 && needle->body.storage_type == MVM_STRING_GRAPHEME_32) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_8 + start, hgraphs - start, needle->body.storage.blob_32, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("832 r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
    }
	else if (haystack->body.storage_type == MVM_STRING_GRAPHEME_ASCII && needle->body.storage_type == MVM_STRING_GRAPHEME_ASCII) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_ascii + start, hgraphs - start, needle->body.storage.blob_ascii, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("aa r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
    }
	else if (haystack->body.storage_type == MVM_STRING_GRAPHEME_ASCII && needle->body.storage_type == MVM_STRING_GRAPHEME_8) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_ascii + start, hgraphs - start, needle->body.storage.blob_8, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("a8 r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
    }
	else if (haystack->body.storage_type == MVM_STRING_GRAPHEME_ASCII && needle->body.storage_type == MVM_STRING_GRAPHEME_32) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_ascii + start, hgraphs - start, needle->body.storage.blob_32, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("a32 r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
    }
	else if (haystack->body.storage_type == MVM_STRING_GRAPHEME_32 && needle->body.storage_type == MVM_STRING_GRAPHEME_32) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_32 + start, hgraphs - start, needle->body.storage.blob_32, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("32 r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
    }
	else if (haystack->body.storage_type == MVM_STRING_GRAPHEME_32 && needle->body.storage_type == MVM_STRING_GRAPHEME_8) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_32 + start, hgraphs - start, needle->body.storage.blob_8, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("328 r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
    }
	else if (haystack->body.storage_type == MVM_STRING_GRAPHEME_32 && needle->body.storage_type == MVM_STRING_GRAPHEME_ASCII) {
        if (start < hgraphs) {
            result = (MVMint64)boyer_moore(haystack->body.storage.blob_32 + start, hgraphs - start, needle->body.storage.blob_ascii, ngraphs);
            if (result > -1) result += start;
            if (result > 500) printf("32a r: %i, s: %i\n", result, start);
        }
        else {
            result = -1;
        }
